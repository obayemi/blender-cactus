...
create table public.invitations (
    ...
    consumed_by_id integer not null references public.users(id) on delete CASCADE,

    UNIQUE (consumed_by_id)
);
...
create index invitation_created_by_id on public.invitations(created_by_id);
create index invitation_consumed_by_id on public.invitations(consumed_by_id);

comment on constraint invitations_created_by_id_fkey on public.invitations is
  E'@foreignFieldName invitations_created';

comment on constraint invitations_consumed_by_id_fkey on public.invitations is
  E'@foreignSingleFieldName invitation';
